...
class IouLoss(nn.Module):
    ...
    def forward(self, pred, target):  # (xi.yi,w,h)
        ...
        if self.loss_type == "iou":
            ...
            inter_min_xy = torch.min((pred[:, :2] - pred[:, 2:] / 2), (target[:, :2] - target[:, 2:] / 2))
            inter_max_xy = torch.max((pred[:, :2] + pred[:, 2:] / 2), (target[:, :2] + target[:, 2:] / 2))
            area_c = torch.prod(inter_max_xy - inter_min_xy, 1)
            giou = iou - (area_c - area_u) / area_c.clamp(1e-7)
            loss = 1 - giou.clamp(min=-1.0, max=1.0)

        elif self.loss_type == "diou":
            inter_min_xy = torch.min((pred[:, :2] - pred[:, 2:] / 2), (target[:, :2] - target[:, 2:] / 2))  # 包围框的左上点
            inter_max_xy = torch.max((pred[:, :2] + pred[:, 2:] / 2), (target[:, :2] + target[:, 2:] / 2))  # 包围框的右下点

            convex_dis = torch.pow(inter_max_xy[:, 0] - inter_min_xy[:, 0], 2) + \
                         torch.pow(inter_max_xy[:, 1] - inter_min_xy[:, 1], 2) + 1e-7  # convex diagonal squared
            ...
            C_w = torch.pow(inter_max_xy[:, 0] - inter_min_xy[:, 0], 2) + 1e-7  # 包围框的w平方
            C_h = torch.pow(inter_max_xy[:, 1] - inter_min_xy[:, 1], 2) + 1e-7  # 包围框的h平方
...
class AlphaIouLoss(nn.Module):
    ...
    def __init__(self, reduction="mean", loss_type="ciou", alpha=3):
    ...
    def forward(self, pred, target):  # (x,y,w,h)
        '''
        :param pred: tensor, [bs,4],xywh
        :param target: tensor, [bs,4] xywh
        :return:
        ...
        if self.loss_type == "iou":
            ...
            convex_dis = torch.pow(inter_max_xy[:, 0] - inter_min_xy[:, 0], 2) + \
                         torch.pow(inter_max_xy[:, 1] - inter_min_xy[:, 1], 2) + 1e-7
            center_dis = (torch.pow(pred[:, 0] - target[:, 0], 2) + torch.pow(pred[:, 1] - target[:, 1], 2))

            diou = iou ** self.alpha - (center_dis ** self.alpha / convex_dis ** self.alpha)
            loss = 1 - diou.clamp(min=-1.0, max=1.0)
